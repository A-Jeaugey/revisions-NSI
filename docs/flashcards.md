...
(function () {
  const CARDS = [
    ...
    { tag: "04", q: "Quels sont les 4 termes du vocabulaire POO au programme NSI Terminale ?", a: "<strong>Classe</strong>, <strong>attribut</strong>, <strong>méthode</strong>, <strong>objet</strong>. Le BO précise : « On n'aborde pas ici tous les aspects de la POO comme le polymorphisme et l'héritage. »" },
    ...
    { tag: "07", q: "Différence INNER JOIN / LEFT JOIN en SQL ?", a: "INNER JOIN : ne garde que les paires de lignes appariées. LEFT JOIN : garde toutes les lignes de gauche, NULL si pas de correspondance à droite." },
    ...
    { tag: "07", q: "Ordre d'exécution logique d'une requête SELECT (au programme NSI) ?", a: "FROM → JOIN → WHERE → SELECT → ORDER BY → LIMIT. (GROUP BY/HAVING sont hors programme NSI.)" },
    ...
    { tag: "09", q: "Comment un processus est-il créé ?", a: "Un processus parent demande à l'OS de créer un processus enfant : l'OS lui attribue un PID, alloue son espace mémoire, et le place en état Prêt." },
    ...
    { tag: "11", q: "Quels sont les protocoles de routage au programme NSI Terminale ?", a: "<strong>RIP</strong> (métrique : nombre de sauts, algorithme Bellman-Ford) et <strong>OSPF</strong> (métrique : coût, algorithme Dijkstra)." },
    { tag: "11", q: "Rôle de DNS ?", a: "Traduit un nom de domaine (<code>exemple.fr</code>) en adresse IP." },
    { tag: "11", q: "Modèle TCP/IP : combien de couches et lesquelles ?", a: "4 couches : Application, Transport, Internet, Accès réseau. (OSI = modèle théorique 7 couches, mais TCP/IP = modèle pratique.)" },
    ...
    { tag: "14", q: "Principe du masque jetable (one-time pad) ?", a: "Chiffrement XOR avec une clé <strong>aléatoire</strong>, de <strong>même longueur</strong> que le message, utilisée <strong>une seule fois</strong> → sécurité parfaite (Shannon)." },
    { tag: "14", q: "Comment HTTPS combine-t-il chiffrement asymétrique et symétrique ?", a: "L'asymétrique sert à <strong>partager</strong> une clé symétrique sans qu'elle soit interceptée. Ensuite, on chiffre les échanges avec cette clé symétrique (rapide)." },
    ...
    { tag: "15", q: "Architecture de von Neumann en 1 phrase ?", a: "CPU + mémoire (instructions ET données) + périphériques, reliés par des bus." },
    { tag: "15", q: "Cite 2 avantages de l'intégration des composants en un SoC.", a: "Plus compact, <strong>plus rapide</strong> (communications internes plus courtes), <strong>moins gourmand en énergie</strong>." },
    { tag: "15", q: "Qu'est-ce qu'un SoC ?", a: "System on Chip : circuit intégré regroupant CPU, mémoire, contrôleurs (et parfois GPU) sur une même puce." },
    { tag: "16", q: "Complexité de la recherche naïve d'un motif de longueur m dans un texte de longueur n ?", a: "O(n × m) dans le pire cas." },
    { tag: "16", q: "Quelles sont les deux idées clés de Boyer-Moore ?", a: "(1) Comparer le motif <strong>de droite à gauche</strong>. (2) Utiliser la règle du <strong>mauvais caractère</strong> pour sauter plusieurs positions d'un coup quand un caractère du texte n'est pas dans le motif." },
    { tag: "17", q: "Cite les 4 paradigmes principaux et un langage par paradigme.", a: "Impératif (C), Orienté objet (Java), Fonctionnel (Haskell), Déclaratif (SQL, Prolog)." },
    { tag: "17", q: "Différence impératif / déclaratif ?", a: "Impératif : on décrit COMMENT obtenir le résultat (séquence d'instructions). Déclaratif : on décrit QUOI obtenir, le moteur s'occupe du comment." },
